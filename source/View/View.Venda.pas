...
begin
  if Copy(cdsConsultaVendaStatus.AsString,1,1) = 'E' then begin
    Application.MessageBox('Venda efetivada não pode ser alterada!', '', MB_OK+MB_ICONINFORMATION);
    Exit;
  end;
...
begin
  if Copy(cdsConsultaVendaStatus.AsString,1,1) = 'E' then begin
    Application.MessageBox('Venda efetivada não pode ser deletada!', '', MB_OK+MB_ICONINFORMATION);
    Exit;
  end;
...
begin
  if cdsItens.Locate('Codigo', dblcProduto.KeyValue, []) then begin
    Application.MessageBox('Produto já inserido!', '', MB_OK+MB_ICONERROR);
    Exit;
  end;
